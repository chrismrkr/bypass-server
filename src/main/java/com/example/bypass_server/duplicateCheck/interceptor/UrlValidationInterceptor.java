...
public class UrlValidationInterceptor implements AsyncHandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (requestType != null && requestType instanceof ValidationType) {
            ...
            if (distributedLockStorage.isUrlHeld(token, requestURI)) {
                return false;
            }
            distributedLockStorage.isUrlHeld(token, requestURI);
        }
        return true;
    }
    ...
    public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, @Nullable Exception ex) throws Exception {
        ...
        if (requestType != null && requestType instanceof ValidationType) {
            ...
            String token = request.getHeader(tokenHeader);
            String requestURI = request.getRequestURI();
            distributedLockStorage.releaseUrl(token, requestURI);
        }
        return;
    }
}
